...
public class FileControlMgr {
    ...
    public void saveBitmapToFileCache(Bitmap bitmap, String strFilePath) {
        ...
        Log.d(com.kiki.android.Utils.conf.Log.LOG_NAME, this.getClass() + " @@ Path : " + fileCacheItem.getPath());
        Log.d(com.kiki.android.Utils.conf.Log.LOG_NAME, this.getClass() + " @@ 이미지 파일 존재 여부 : " + fileCacheItem.exists());
        if (!fileCacheItem.exists()) {
            File folder = new File(ex_storage);
            folder.mkdirs();
        } else {//기존에 있으면
            android.util.Log.d(com.kiki.android.Utils.conf.Log.LOG_NAME, this.getClass() + " @@ 이미지 기존 파일 삭제후 다시쓰기");
        ...
        try {
            ...
            android.util.Log.d(com.kiki.android.Utils.conf.Log.LOG_NAME, this.getClass() + " @@ 이미지  파일 생성완료 ");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
